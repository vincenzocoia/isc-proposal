...
# Resolve to a specific distribution; perhaps evaluate the cdf:
...
Through an iterative design process, I have come up with the following design:
...
- Distribution parameters (like the mean and variance of the Normal distribution) might not be resolved to specific values. Once resolved, objects in the distribution environment can also be resolved.
  - Lazy or active bindings sound like appealing options for the initial definition of the distribution environment, or incorporating parameters in a data mask, but issues arise with the order in which properties are accessed.
...
A minimum viable product includes improvements to the existing **distionary** package so as to submit to CRAN. A simpler design can be permitted in case not all the goals (listed in the previous section) are achieved.